...
- **Date handling**: Every TIMESTAMP column stores **local wall-clock time as a naive value** — the importers never convert to UTC. XML attributes and ECG CSV headers arrive as `"YYYY-MM-DD HH:MM:SS +OOOO"` (local time + offset); `clean_date` in `xml.rs` (and the inline equivalent in `ecg.rs`) drops the offset suffix and stores the local half. The workout's offset is preserved on `workouts.start_offset_minutes` so it can be recovered later. GPX route timestamps arrive in true UTC (`"...Z"`); the GPX importer shifts them by the owning workout's offset via `shift_utc_to_local` in `gpx.rs` so route points land on the same local-time basis as everything else. Workouts without a parseable offset fall back to a strip-only path that preserves the legacy (pre-fix) wall-clock value.
