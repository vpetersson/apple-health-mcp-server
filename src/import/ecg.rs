...
const RECORDED_DATE_LABELS: &[&str] = &[
    "Recorded Date",
    // Apple emits one of two Japanese spellings depending on iOS/watchOS
    // version. Both are verified against real exports.
    "記録日",   // ja, verified (current iOS in JP)
    "記録日時", // ja, verified (older iOS in JP)
    // unverified below
    "记录日期", // zh-Hans
    "記錄日期", // zh-Hant
    "기록 일시", // ko
...
pub(crate) fn import_single_ecg(conn: &Connection, path: &Path, import_id: &str) -> Result<()> {
    ...
    for line in lines.by_ref() {
            ...
            || match_header(line, DOB_LABELS).is_some()
        {
            ...
            // Extract the leading numeric span: handles both
            //   "513.992 hertz"  (English, space-separated)
            //   "512ヘルツ"      (Japanese, no separator)
            //   "512Hz"          (compact form)
            // by collecting digits + '.' until the first non-numeric char.
            let numeric: String = raw
                .chars()
                .take_while(|c| c.is_ascii_digit() || *c == '.')
                .collect();
            sample_rate_hz = numeric.parse().ok();
...
75";
    ...
    // by an older Japanese-language Apple Watch (which writes "記録日時" and
    // "512.000 Hz"). Tests that the i18n matcher accepts them.
...
75";
    ...
    // Japanese-locale ECG CSV as written by current iOS in JP: the recorded
    // date label drops the trailing "時" and the sample rate uses the
    // Japanese unit "ヘルツ" with no separator. Verified against a real
    // export on watchOS 10.x.
    const JAPANESE_ECG_CSV_CURRENT: &str = "名前,テスト ユーザー
生年月日,\"1990/01/01\"
...
75";
    ...
    // Regression: the "current iOS in JP" labels (記録日 instead of 記録日時,
    // and "512ヘルツ" with no space before the unit) used to fall through to
    // the "No recorded date" bail and silently NULL the sample rate.
    #[test]
    fn import_single_ecg_japanese_locale_current() {
        let conn = open_db_in_memory().unwrap();
        ensure_schema(&conn).unwrap();

        let dir = tempfile::tempdir().unwrap();
        let ecg_path = dir.path().join("ecg_ja_current.csv");
        std::fs::write(&ecg_path, JAPANESE_ECG_CSV_CURRENT).unwrap();
        ...
        let (classification, device, sample_rate): (String, String, f64) = conn
            .query_row(
                "SELECT classification, device, sample_rate_hz FROM ecg_readings LIMIT 1",
                [],
                |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
            )
            .unwrap();
        assert_eq!(classification, "洞調律");
        assert_eq!(device, "Watch7,1");
        assert!(
            (sample_rate - 512.0).abs() < 0.01,
            "expected ~512 Hz, got {sample_rate}"
        );
    ...
    fn match_header_picks_first_label() {
        ...
        // Current iOS in JP writes "記録日" (no trailing 時).
        assert_eq!(match_header("記録日,2024-01-01", RECORDED_DATE_LABELS), Some("2024-01-01"));
        ...
        // The 記録日 entry must not greedily eat the 記録日時 value either.
        assert_eq!(
            match_header("記録日時,2024-06-15 10:30:00 +0900", RECORDED_DATE_LABELS),
            Some("2024-06-15 10:30:00 +0900"),
        );
    }

    // The sample-rate parser must accept the formats Apple actually emits:
    // English with a unit suffix ("512.000 Hz", "513.992 hertz") and Japanese
    // with no separator before the unit ("512ヘルツ").
    #[test]
    fn sample_rate_parses_numeric_prefix() {
        ...
        assert_eq!(parse("513.992 hertz"), Some(513.992));
        assert_eq!(parse("512.000 Hz"), Some(512.0));
        assert_eq!(parse("512ヘルツ"), Some(512.0));
        assert_eq!(parse("512Hz"), Some(512.0));
        assert_eq!(parse("ヘルツ"), None);
    }
